...
public class SwerveModule {
    ...
    // Profiled PID controller is an extension of PID controllers that allows for
    // velocity and acceleration constraints
    // These are feedback controllers, so they correct for error
    ProfiledPIDController drivePID = new ProfiledPIDController(3.1679, 0, 0,
            new Constraints(Constants.maxModuleVelocity, Constants.maxModuleVelocity));
     ...
     * 
     * @param driveVolts Voltage to set to the drive motor. Must be from -12.0 to
     *                   12.0
     * @param turnVolts  Voltage to set to the turn motor. Must be from -12.0 to
     *                   12.0
     */
     ...
     * 
     * @return A new SwerveModulePosition with the drive motor's distance and the
     *         turn motor's angle
     */
    ...
    public void setDesiredState(SwerveModuleState desiredState) {
        ...
        // In simple terms, turning 180 degrees and driving forward is the same as
        // turning 90 degrees and drving backwards.
        ...
        // The PID controllers will return very small decimals which will leave the
        // motors at a constant stall.
        // This can damage the motors, so this deadzone is a minimum voltage to be
        // returned to drive the module.
    ...
    private double getDriveVelocity() {
        return driveMotor.getPosition().getValueAsDouble() / Constants.swerveDriveGearRatio
                / Constants.falconEncoderResolution * Math.PI * Constants.swerveWheelDiameter;
    }
    ...
    private double getCurrentAngle() {
        return turnMotor.getPosition().getValueAsDouble() / Constants.swerveTurnGearRatio
                / Constants.falconEncoderResolution
                * 2 * Math.PI;
    }
}